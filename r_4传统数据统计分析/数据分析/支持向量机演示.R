...
summary(iris)
head(iris);dim(iris)
model1=svm(Species~.,data=iris)# 建立svm模型
###第二种格式建立模型
x=iris[,-5]						# 提取iris数据中除第5列以外的数据作为特征变量
y=iris[,5]						# 提取iris数据中的第5列数据作为结果变量
model2=svm(x,y,kernel ="radial",gamma =if(is.vector(x)) 1 else 1/ncol(x))   # 建立svm模型
summary(model1)
summary(model2)
...
head(x)
pred=predict(model1,x)								# 根据模型model对x数据进行预测
pred[sample(1:150,8)]								 # 随机挑选8个预测结果进行展示
table(pred,y)								# 模型预测精度展示

###实际建模过程中完整操作
#在时间和精力允许的情况下，尽可能的建立所有可能的模型
...
plot(cmdscale(dist(iris[,-5])),
     col=c("lightgray","black","red")[as.integer(iris[,5])],    #该分为何种类别就作出何种图形
     pch= c("o","+")[1:150 %in% model$index + 1])                # 绘制模型分类散点图,判断是否为支持向量点
legend(1,-0.5,c("setosa","versicolor","virginica"),col=c("lightgray","black","gray"),lty=.5)		# 标记图例
